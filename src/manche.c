...
bool PrenomValide(char *prenom) {
  ...
  unsigned char *s =
      (unsigned char *)prenom; // S lit chaque octet de prenom comme un nombre entre 0 et 255 
  ...
  while (*s != '\0') {
    ...
    // Une fois qu'on a ce numéro, on peut vérifier si le caractère est dans la plage des lettres autorisées (0x00C0 à 0x017F)
    else if (*s >= 0xC2 && *s <= 0xD9) {
      // On décale les bits du 1er octets de 6 cases vers la gauche pour reserver de la place pour les bits du 2ème octet 
      // En binaire, decaler vers la gauche de N position revient à multiplier la valeur initiale par 2^N
      // << 6 = multiplier par 2^6
      // Exemple : avec bit1 = 3 (Avant decalage : 00000011 | Après le décalage : 11000000)
...
void VoirRegle() {
  int lire = 0, valide = 0;
  do {
    printf(RESET V_BLANC "Voulez-vous consulter les rêgles du jeu avant de "
                         "commencer la partie ?\n- Oui : 1 \n- Non : 0\n");
    if (scanf("%d", &lire) == 1) {
      if (lire == 0 || lire == 1) {
        valide = 1;
...
void nmbJoueurs(int *nbJoueurs) {
  ...
  do {
    printf(RESET V_BLANC "\nCommençons, combien il y a t-il de joueurs dans la "
                         "partie ? (3-100 joueurs) " EMOJI_JOUEUR "\n");
...
bool Doublon(Perso *joueur) {
  ...
  for (unsigned int i = 0; i < joueur->nbcarte; i++) {
    // Si on tombe sur une carte bonus ou speciale on passe à la prochaine itération car seules les cartes numéros comptent comme des doublons
    if (joueur->carte[i].type != 'N') {
      continue;
    }
    ...
    if (Joueurs[joueurActuel].Ajouer == false) {
      ...
      if (Joueurs[joueurActuel].nbcarte >= MAIN) {
        ...
        if (decision == 1) {
          ...
          // Cas 3a : le joueur pioche une carte qu'il a déjà dans sa main -> il est eliminé de la manche et ne marque pas de point
          ...
          // Cas 3c : le joueur décide de s'arrêter volontairement -> il garde le score qu'il avait accumulé jusqu'à préssent 
          //                                                           et sa manche se termine
